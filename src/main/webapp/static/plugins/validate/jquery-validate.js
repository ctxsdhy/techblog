(function(factory) {
    ...
    var RULES = {
        ...
        isSame: function (value, other, errorMsg, el) {
            //是否相同
    ...
    Validator.prototype.showMsg = function(target, msg, status, callback) {
        ...
        // 1 : success
        // 2 : error
        var _current = status ? (status > 1 ? 'input-warning' : 'input-success') : 'input-tip';
        var $context = target;
        var _other = target.attr('data-type') || '';
        $context.removeClass('input-success input-tip input-warning').addClass(_current + ' ' + _other);
        
        var _currentmsg = status ? (status > 1 ? 'input-warning-msg' : 'input-success-msg') : 'input-tip-msg';
        var $msg = $context.parent().find('.input-msg');
        if ($msg.length == 0) {
            $msg = $context.parent().parent().find('.input-msg');
            if ($msg.length == 0) {
                $msg = $context.parent().parent().parent().find('.input-msg');
                if ($msg.length == 0) {
                    $msg = $context.parent().parent().parent().parent().find('.input-msg');
                }
            }
        }
        $msg.removeClass('input-success-msg input-tip-msg input-warning-msg').addClass(_currentmsg).text('').text(msg);
    };
            ...
            $body.on({
                ...
                blur: function(event) {
                    ...
                    if (!errMsg) {
                        ...
                        setting.successTip ? validator.showMsg($this, '', 1) : $this.parent().find('.input-msg').remove();
                    }
