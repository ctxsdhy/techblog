...
<html>
			...
			$(function() {
				...
				$.getJSON("/c/getListj", function(data) {
					...
					if('${categoryInfo.parentId}' != '') {
						$('#categorySelect').val('${categoryInfo.parentId}');
					}
				});
	...
	<body>
		<div class="r-main">
			<form action="${path}/c/s" method="post">
				<div class="r-m-head">
					<div class="r-m-h-title">
						<input type="text" class="form-control" placeholder="分类名称" name="categoryName" value="${categoryInfo.categoryName}" />
						<c:if test="${!empty categoryInfo}">
				...
				<div class="r-m-list">
					<div class="r-m-l-content">
						<script type="text/plain" id="editor" name="categoryContent" style="width: 100%; height: 100%;">${categoryInfo.categoryContent}</script>
					    <script type="text/javascript">
				...
				<div class="r-m-footer">
					<div class="r-m-f-main">
						...
						<a href="${path}/c/m/list" class="btn btn-default btn-xs">返回</a>&nbsp;
						<span style="display:inline-block;float:right;">
							<select id="categorySelect" name="parentId">
								<option value="0">选择分类</option>
							</select>&nbsp;
							<input type="text" style="border:1px solid #ccc;width:30px;padding:1px;" placeholder="序号" name="orderId" value="${categoryInfo.orderId}" /> 
						</span>
					</div>
				</div>
			</form>
